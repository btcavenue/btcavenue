dnl Copyright (c) 2013-2016 The Btcavenue Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set btcavenue_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$btcavenue_qt_want_version" = xauto && test "x$btcavenue_qt_force" != xyes; then
    if test "x$btcavenue_enable_qt" != xno; then
      AC_MSG_WARN([$1; btcavenue-qt frontend will not be built])
    fi
    btcavenue_enable_qt=no
    btcavenue_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$btcavenue_enable_qt" != xno && test "x$btcavenue_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build btcavenue-qt GUI (default=auto)])],
    [
     btcavenue_qt_want_version=$withval
     if test "x$btcavenue_qt_want_version" = xyes; then
       btcavenue_qt_force=yes
       btcavenue_qt_want_version=auto
     fi
    ],
    [btcavenue_qt_want_version=auto])
...
dnl Outputs: btcavenue_enable_qt, btcavenue_enable_qt_dbus, btcavenue_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$btcavenue_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    btcavenue_enable_qt=yes
    btcavenue_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      btcavenue_enable_qt_test=no
    fi
    btcavenue_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      btcavenue_enable_qt_dbus=yes
    fi
  ...
  if test x$btcavenue_enable_qt = xyes; then
    AC_MSG_RESULT([$btcavenue_enable_qt ($QT_LIB_PREFIX)])
  else
    AC_MSG_RESULT([$btcavenue_enable_qt])
  fi
...
dnl Output: btcavenue_cv_qt5=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT5],[
  AC_CACHE_CHECK(for Qt 5, btcavenue_cv_qt5,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [btcavenue_cv_qt5=yes],
    [btcavenue_cv_qt5=no])
])])

dnl Internal. Check if the included version of Qt is greater than Qt58.
dnl Requires: INCLUDES must be populated as necessary.
dnl Output: btcavenue_cv_qt58=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, btcavenue_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [btcavenue_cv_qt58=yes],
    [btcavenue_cv_qt58=no])
])])
...
dnl Output: btcavenue_cv_static_qt=yes|no
dnl Output: Defines QT_STATICPLUGIN if plugins are static.
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, btcavenue_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
      ...
      [btcavenue_cv_static_qt=yes],
      [btcavenue_cv_static_qt=no])
    ])
  if test "x$btcavenue_cv_static_qt" = xyes; then
...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$btcavenue_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, btcavenue_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
           ...
           [btcavenue_cv_need_platformsupport=yes],
           [btcavenue_cv_need_platformsupport=no])
         ])
         if test "x$btcavenue_cv_need_platformsupport" = xyes; then
           if test x$btcavenue_cv_qt58 = xno; then
...
dnl Inputs: btcavenue_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If btcavenue_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: btcavenue_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$btcavenue_qt_want_version" = xauto; then
