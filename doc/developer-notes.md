...
- [Development guidelines](#development-guidelines)
    - [General Btcavenue Core](#general-btcavenue-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Btcavenue Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play btcavenues" on the test network, if you
...
Btcavenue Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/btcavenue/btcavenue/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_btcavenue
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_btcavenue --log_level=test_suite
$ valgrind -v --leak-check=full src/btcavenued -printtoconsole
...
# A coverage report will now be accessible at `./test_btcavenue.coverage/index.html`.
```
...
To profile a running btcavenued process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep btcavenued` -- sleep 60
```
...
Btcavenue Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/btcavenue/btcavenue/issues/12691)
...
However, in open source software such as Btcavenue Core, where everyone uses
...
pay attention to for reviewers of Btcavenue Core code.

General Btcavenue Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Btcavenue Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Btcavenue Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Btcavenue Core so that they can be integrated
...
- src/secp256k1
  - Upstream at https://github.com/btcavenue-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/btcavenue-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/btcavenue-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with Btcavenue's `select()` loop, because
...
compatibility between nodes. This happened in Btcavenue 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Btcavenue's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/btcavenue/btcavenue/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/btcavenue/btcavenue/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/btcavenue/btcavenue/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `btcavenue-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `btcavenue-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Btcavenue Core, the wallet was always
