Btcavenue Core version 0.18.0 is now available from:

  <https://btcavenuecore.org/bin/btcavenue-core-0.18.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://btcavenuecore.org/en/list/announcements/join/>
...
`/Applications/Btcavenue-Qt` (on Mac) or `btcavenued`/`btcavenue-qt` (on
Linux).
...
Btcavenue Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Btcavenue Core on unsupported systems.

Btcavenue Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Btcavenue Core does not yet change appearance when
...
`contrib/init/btcavenued.service`, it has been changed to use
`/var/lib/btcavenued` as the data directory instead of
`~btcavenue/.btcavenue`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/btcavenued` will
exist has enough space (check using `df -h /var/lib/btcavenued`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/btcavenue/btcavenue/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://btcavenuecore.org/en/contact/) the Btcavenue Core
...
The systemd init file (`contrib/init/btcavenued.service`) has been changed
to use `/var/lib/btcavenued` as the data directory instead of
`~btcavenue/.btcavenue`. This change makes Btcavenue Core more consistent with
...
`datadir` option in `/etc/btcavenue/btcavenue.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/btcavenue/btcavenue.conf`.


Documentation
-------------

- A new short [document](https://github.com/btcavenue/btcavenue/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/btcavenue/btcavenue/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/btcavenue/btcavenue/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/btcavenue/btcavenue/blob/master/doc/btcavenue-conf.md)
  about the `btcavenue.conf` file describes how to use it to configure
  Btcavenue Core.

- A new document introduces Btcavenue Core's BIP174 [Partially-Signed
  Btcavenue Transactions
  (PSBT)](https://github.com/btcavenue/btcavenue/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/btcavenue/btcavenue/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Btcavenue-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Btcavenue Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/btcavenue/btcavenue/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/btcavenue/btcavenue/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Btcavenue Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Btcavenue Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/btcavenue/btcavenue/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/btcavenue/btcavenue/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart btcavenued with the `-deprecatedrpc=generate`
...
- A new `btcavenue-wallet` tool is now distributed alongside Btcavenue
...
This section describes planned changes to Btcavenue Core that may affect
other Btcavenue software and services.

- Since version 0.16.0, Btcavenue Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Btcavenue Core 0.20 (expected about
  a year after 0.18), Btcavenue Core will default to native segwit
  ...
  to bech32 addresses, and if the Btcavenue Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Btcavenue Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Btcavenue Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Btcavenue Core
...
- A sub-project of Btcavenue Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Btcavenue Core.  See their [project
  page](https://github.com/btcavenue-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Btcavenue Core's own implementation, although entropy
  gathered by Btcavenue Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Btcavenue Core a
...
- On macOS, Btcavenue Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `btcavenue_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbtcavenue_server.a` (jnewbery)
...
- #13926 [Tools] btcavenue-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like btcavenued (MarcoFalke)
...
- #14521 qt, docs: Fix `btcavenue-qt -version` output formatting (hebasto)
...
- #11625 Add BtcavenueApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbtcavenue_server` (laanwj)
...
- #15154 Configure: btcavenue-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/btcavenue-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Btcavenue Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbtcavenue_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in btcavenue-util-test.py (isghe)
...
- #12775 Integration of property based testing into Btcavenue Core (Christewart)
...
- #12246 Bugfix: Only run btcavenue-tx tests when btcavenue-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_btcavenue` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_btcavenue` (MarcoFalke)
...
- #14186 btcavenue-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_btcavenue.conf` message on startup if conf file exists (leishman)
...
- #12255 Update btcavenue.service to conform to init.md (dongcarl)
...
- #14497 Add doc/btcavenue-conf.md (hebasto)
...
- #15354 Add missing `btcavenue-wallet` tool manpages (MarcoFalke)
...
- #15765 correct btcavenueconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support btcavenue folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/btcavenue/).
