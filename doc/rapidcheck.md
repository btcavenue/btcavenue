# RapidCheck property-based testing for Btcavenue Core

## Concept

Property-based testing is experimentally being added to Btcavenue Core with
...
If RapidCheck is installed, Btcavenue Core will automatically run the
...
3. Configure Btcavenue Core with RapidCheck.

    `cd` to the directory of your local btcavenue repository and run
...
4. Build Btcavenue Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_btcavenue-crypto_gen.o
    CXX      test/test_btcavenue-key_properties.o
    ```
...
That's it! You are now running property-based tests in Btcavenue Core.
