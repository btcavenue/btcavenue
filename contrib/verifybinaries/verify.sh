...
###   btcavenuecore.org and btcavenue.org and compares them.
...
WORKINGDIR="/tmp/btcavenue_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://btcavenuecore.org"
HOST2="https://btcavenue.org"
BASEDIR="/bin/"
VERSIONPREFIX="btcavenue-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'btcavenue-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "btcavenue.org failed to provide signature file, but btcavenuecore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "btcavenue.org and btcavenuecore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Btcavenue Core binary release signing key installed?"
   fi
