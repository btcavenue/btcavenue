  ...
  do
    ...
    if [ "$DOWNLOAD_BINARY" -eq "0" ]; then
      ...
      if [ ! -d "$tag" ]; then
        ...
        git clone https://github.com/btcavenue/btcavenue "$tag"
        pushd "$tag" || exit 1
        {
          git checkout "$tag"
          ...
          mkdir bin
          mv src/btcavenued src/btcavenue-cli src/btcavenue-tx bin
          if [ "$FUNCTIONAL_TESTS" -eq "0" ]; then
            mv src/qt/btcavenue-qt bin
          fi
        }
        popd || exit 1
      fi
      ...
      if [ -d "$tag" ]; then
        ...
        if [[ "$tag" =~ v(.*)(rc[0-9]+)$ ]]; then
            BIN_PATH="bin/btcavenue-core-${BASH_REMATCH[1]}/test.${BASH_REMATCH[2]}"
        else
            BIN_PATH="bin/btcavenue-core-${tag:1}"
        fi
        URL="https://btcavenue.org/$BIN_PATH/btcavenue-${tag:1}-$PLATFORM.tar.gz"
        echo "Fetching: $URL"
        curl -O $URL
        tar -zxf "btcavenue-${tag:1}-$PLATFORM.tar.gz" -C "$tag" --strip-components=1 "btcavenue-${tag:1}"
        rm "btcavenue-${tag:1}-$PLATFORM.tar.gz"
      fi
    fi
  done
}
