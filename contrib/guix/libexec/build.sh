...
# Setup the directory where our Btcavenue Core build for HOST will occur
...
make -C depends --jobs="$MAX_JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform btcavenue-linux-g++'
...
if [ -z "$(find "${OUTDIR}/src" -name 'btcavenue-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'btcavenue-*.tar.gz')" "${OUTDIR}/src/"
fi

# Determine the full path to our source tarball
SOURCEDIST="$(find "${OUTDIR}/src" -name 'btcavenue-*.tar.gz')"
# Determine our distribution name (e.g. btcavenue-0.18.0)
...
(
    ...
    # Setup the directory where our Btcavenue Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Btcavenue Core to $INSTALLPATH
