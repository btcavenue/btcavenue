// Copyright (c) 2018 The Btcavenue Core developers
...
// btcavenue includes.
#include <..\src\script\btcavenueconsensus.h>
...
int main()
{
    std::cout << "btcavenueconsensus version: " << btcavenueconsensus_version() << std::endl;
    ...
    auto op0Result = btcavenueconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, btcavenueconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
