 ...
 */
static const char* BITCOIN_PID_FILENAME = "btcavenued.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/btcavenue/btcavenue>";
    const std::string URL_WEBSITE = "<https://btcavenuecore.org>";
...
/** Sanity checks
 *  Ensure that Btcavenue is running in a usable environment with all
...
{
    // Make sure only a single Btcavenue process is using the data directory.
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if btcavenue is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if btcavenue is started while in a temporary directory.\n",
