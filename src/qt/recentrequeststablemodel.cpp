    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return BtcavenueUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, BtcavenueUnits::separatorNever);
            else
                return BtcavenueUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+BtcavenueUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
