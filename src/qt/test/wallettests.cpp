// Copyright (c) 2015-2019 The Btcavenue Core developers
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<BtcavenueAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_btcavenue-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_btcavenue-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_btcavenue-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = BtcavenueUnits::formatWithUnit(unit, balance, false, BtcavenueUnits::separatorAlways);
    ...
    // Amount input
    BtcavenueAmountField* amountInput = receiveCoinsDialog.findChild<BtcavenueAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: btcavenue:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_btcavenue-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
