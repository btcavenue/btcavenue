...
const char * const BITCOIN_CONF_FILENAME = "btcavenue.conf";
 ...
 * https://github.com/btcavenue/btcavenue/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //btcavenue-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "btcavenue";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Btcavenue
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Btcavenue
    // Mac: ~/Library/Application Support/Btcavenue
    // Unix: ~/.btcavenue
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Btcavenue";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Btcavenue";
#else
    // Unix
    return pathRet / ".btcavenue";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Btcavenue Core copyright is not removed by accident
    if (copyright_devs.find("Btcavenue Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Btcavenue Core developers";
    }
