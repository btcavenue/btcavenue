...
# TestP2PConn: A peer we use to send messages to btcavenued, and store responses.
...
class CompactBlocksTest(BtcavenueTestFramework):
    ...
    # This test actually causes btcavenued to (reasonably!) disconnect us, so do this last.
    ...
    # Test that btcavenued requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for btcavenued to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # btcavenued will not send blocktxn responses for blocks whose height is
