...
from test_framework.test_framework import BtcavenueTestFramework, SkipTest
...
class BackwardsCompatibilityTest(BtcavenueTestFramework):
    ...
    def setup_nodes(self):
        ...
        self.add_nodes(self.num_nodes, extra_args=self.extra_args, versions=[
            ...
            self.options.btcavenued,
            self.options.btcavenued,
            releases_path + "/v0.19.0.1/bin/btcavenued",
            releases_path + "/v0.18.1/bin/btcavenued",
            releases_path + "/v0.17.1/bin/btcavenued"
        ], binary_cli=[
            self.options.btcavenuecli,
            self.options.btcavenuecli,
            releases_path + "/v0.19.0.1/bin/btcavenue-cli",
            releases_path + "/v0.18.1/bin/btcavenue-cli",
            releases_path + "/v0.17.1/bin/btcavenue-cli"
        ])
    ...
    def run_test(self):
        ...
        # RPC loadwallet failure causes btcavenued to exit, in addition to the RPC
        # call failure, so the following test won't work:
        # assert_raises_rpc_error(-4, "Wallet loading failed.", node_v17.loadwallet, 'w3_v18')

        # Instead, we stop node and try to launch it with the wallet:
        self.stop_node(self.num_nodes - 1)
        node_v17.assert_start_raises_init_error(["-wallet=w3_v18"], "Error: Error loading w3_v18: Wallet requires newer version of Btcavenue Core")
        node_v17.assert_start_raises_init_error(["-wallet=w3"], "Error: Error loading w3: Wallet requires newer version of Btcavenue Core")
        ...
        # Copy the 0.17 wallet to the last Btcavenue Core version and open it:
        ...
        # Copy the 0.19 wallet to the last Btcavenue Core version and open it:
        shutil.copytree(
